...
class Genesis_Latest_Tweets extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'latest-tweets', __( 'Genesis Latest Tweets', GLTW_DOMAIN ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		if ( ! $tweets ) {
			...
			$count           = isset( $instance['twitter_hide_replies'] ) ? (int) $instance['twitter_num'] * 10 : (int) $instance['twitter_num'];
		...
		$follow_link = ($instance['follow_link_show'] && $instance['follow_link_text']) ? '<li class="last"><a href="' . esc_url( 'http://twitter.com/'.$instance['twitter_id'] ).'">'. esc_html( $instance['follow_link_text'] ) .'</a></li>' : '';
	...
	function form( $instance ) {
		...
		if ( empty( $consumer_key ) && empty( $consumer_secret ) && empty( $access_key ) && empty( $access_secret ) ) {
			printf(
				...
				__( 'Genesis Latest Tweets Settings', GLTW_DOMAIN )
			);
			return;
		}
